...
The normal (or Gaussian) distribution (Section 5.7 of the [STAT0002 notes](https://moodle.ucl.ac.uk/course/view.php?id=8579&section=3)) is a symmetric probability distribution.  If $Y = \ln X$ has a normal distribution then $X$ has a *log-normal* distribution.  This is a convenient example to study because if we know that we have a sample from a log-normal distribution then taking logs of the data produces a sample from a normal distribution.
...
[Stochastic simulation video](https://moodle.ucl.ac.uk/course/view.php?id=8579&section=8) for details. 
...
In this example it can be shown that using $\lambda = 2$ will produce a random variable whose p.d.f. is constant (and equal to 2) over the range (-1/2, 0), that is, we produce a uniform U(-1/2, 0) random variable (see Section 5.5 of the [STAT0002 notes](https://moodle.ucl.ac.uk/course/view.php?id=8579&section=3)).  You will be able to prove this after you have done Question 1 of [Exercises 6](https://moodle.ucl.ac.uk/mod/resource/view.php?id=316125).  Question 5 of the [Quiz for Exercises 6](https://moodle.ucl.ac.uk/mod/quiz/view.php?id=316126) also uses this method.
...
In Chapter 8 of the [STAT0002 notes](https://moodle.ucl.ac.uk/course/view.php?id=8579&section=3)) we study simple linear regression.  In short, the idea is to suppose that the distribution of a *response variable* $Y$ has a mean that is a linear function of the observed value $x$ of an *explanatory variable $X$.  Alternatively, we could think of this as $Y$ being a linear function of $x$ plus some random error that has a zero mean.  
