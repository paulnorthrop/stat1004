...
The normal (or Gaussian) distribution (Section 5.7 of the [STAT1004 notes](https://moodle.ucl.ac.uk/mod/resource/view.php?id=2079737)) is a symmteric probability distribution.  If $Y = \ln X$ has a normal distribution then $X$ has a *log-normal* distribution.  This is a convenient example to study because if we know that we have a sample from a log-normal distribution then taking logs of the data produces a sample from a normal distribution.
...
1. **I suggest that try different values of `lambda` and look at the result in the plot.**
...
An exponential random variable (Section 5.6 of the [STAT1004 notes](https://moodle.ucl.ac.uk/mod/resource/view.php?id=2079737)) exhibits strong positive skewness.  Therefore, if we wish to Box-Cox transform data that have been sampled from an exponential distribution in order to make it more symmetric then we should use $\lambda < 1$.  We can use the MASS function `boxcox` to help us to decide which value of $\lambda$ we could use.  However, an exponential distribution is a special case of a gamma distribution (we don't study this in STAT1004) and it has long been known  (@WH1931) that a cube root transformation tends to make a gamma random variable behave rather like a normal random variable.  Therefore, $\lambda = 1/3$ might work well.
...
p.d.f. and to simulate from this distribution, respectively.  The latter uses the *inversion method* of simulation. See the 
[Stochastic simulation video](https://moodle.ucl.ac.uk/mod/lti/view.php?id=3081645) for details. 
...
In this example it can be shown that using $\lambda = 2$ will produce a random variable whose p.d.f. is constant (and equal to 2) over the range (-1/2, 0), that is, we produce a uniform U(-1/2, 0) random variable (see Section 5.5 of the [STAT1004 notes](https://moodle.ucl.ac.uk/mod/resource/view.php?id=2079737)).  You will be able to prove this after you have done Question 1 of [Exercises 6](https://moodle.ucl.ac.uk/mod/resource/view.php?id=2662627).  Guestion 5 of the [Quiz for Exercises 6](https://moodle.ucl.ac.uk/mod/quiz/view.php?id=2671329) also uses this method.

3. **(After we reach Exercises 6 in the course) can you prove that if a random variable $X$ has the triangular p.d.f. above then $X^2$ has a standard U(0,1) distribution and hence that $(X^2-1)/2$ has a U(-1/2,0) distribution?**

We simulate same data from this triangular distribution and show that using $\lambda = 2$ does indeedm produce a density that is flat, and therefore symmetric.
...
In Chapter 8 of the [STAT1004 notes](https://moodle.ucl.ac.uk/mod/resource/view.php?id=2079737)) we study simple linear regression.  In short, the idea is to suppose that the distribution of a *response variable* $Y$ has a mean that is a linear function of the observed value $x$ of an *explanatory variable $X$.  Alternatively, we could think of this as $Y$ being a linear function of $x$ plus some random error that has a zero mean.  
