...
This vignette contains ideas and techniques that are not covered until later in STAT1004 and others (stochastic simulation and logistic regression) that are not part of the STAT1004 syllabus. Therefore, you should not worry too much about these details: just try to get a sense of how the R code works.  The questions and comments below are there to prompt you to think about things.

For information about R see [STAT1006 Moodle page](https://moodle.ucl.ac.uk/course/view.php?id=7596) (if you are taking STAT1006) or [Introduction to R Moodle page](https://moodle.ucl.ac.uk/course/view.php?id=37877) (if you are not taking STAT1006, e.g. Natural Sciences students).

The R code used in this vignette are available [here](stat1004-ch1a-shuttle-vignette.R).  The functions `shuttle_sim` and `shuttle_sim_plot` can be viewed either by typing the name of the function at R command prompt `>` or at  [GitHub](https://github.com/paulnorthrop/stat1004/tree/master/R/shuttle_sim.R)

If you have any questions please ask them using the [STAT1004 discussion forum](https://moodle.ucl.ac.uk/mod/hsuforum/view.php?id=1410864).

## The Challenger Space Shuttle Disaster

In the first lecture of STAT1004 we discussed the Challenger Space Shuttle Disaster.   In this vignette we are concerned mainly with the R code, so there is very little discussion of the problem, the data or statistical ideas.  For discussion of these see the notes and/or lecture slides in the [Notes](https://moodle.ucl.ac.uk/course/view.php?id=3419&section=3) section of the STAT1004 Moodle page.

**Please refer to the discussion of this plot in the lecture slides.**

In this vignette we consider how R can be used to analyse data related to this disaster.  See @shuttle for more information about these data and analyses to estimate the probability of a catastrophic failure of the Challenger space shuttle under the launch conditions on 28th January 1986.  This paper is used as a [worked example](https://moodle.ucl.ac.uk/mod/resource/view.php?id=2125123) for the [STAT1004 Meet your Professor ICA](https://moodle.ucl.ac.uk/course/view.php?id=3419&section=4).
...
The data are available in a **data frame** called `shuttle`.  A data frame is what R calls a table of data.
```{r}
...
This section involves a statistical model, a *logistic regression model*, that is not part of the STAT1004 syllabus, so don't worry too much about the details that follow.  However, this section involves things that we *will* study later in STAT1004, namely *independence*, the *binomial distribution* and (the general concept of) *regression*.  We fit the model using a general fitting method called *maximum likelihood estimation*.  This method is not formally part of the STAT1004 syllabus, but we will look at it very briefly later in the course.

Consider the six O-rings on the space shuttle.  We suppose that on a launch conducted at temperature $t$ degrees F:
...
Under these assumptions the number of O-rings that suffer thermal distress has a binomial distribution with parameters $6$ and $p(t)$.  We will study this distribution in Section 5.3 of the [STAT1004 notes](https://moodle.ucl.ac.uk/pluginfile.php/3054127/mod_resource/content/22/STAT1004notes-2x1.pdf).  We have reason to believe that $p(t)$ depends on $t$, that is, the probability that an O-ring suffers thermal distress is different for different launch temperatures.  A simple model that is used in this kind of situation is a *linear logistic regression model*, in which 
...
11. **Can you gues what the function `seq` does?  Use `?seq` to find out.**
