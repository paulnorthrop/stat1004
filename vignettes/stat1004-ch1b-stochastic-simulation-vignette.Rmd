...
In this vignette we concentrate on using R to perform stochastic simulation.  For a more general introduction to stochastic simulation, and a more detailed explanation of the ideas underlying the R code, see the short [Stochastic simulation video](https://moodle.ucl.ac.uk/mod/url/view.php?id=2180273).  If you want to know more then good sources of information are @Morgan and @Jasra.

The R code used in this vignette are available [here](stat1004-ch1b-stochastic-simulation-vignette.R).
 The function `rbinomial` can be viewed either by typing `rbinomial` at R command prompt `>` or at  [GitHub](https://github.com/paulnorthrop/stat1004/tree/master/R/stochastic_simulation.R)

## Pseudo-random numbers in (0,1)

The basic building block of stochastic simulation methods is the ability to simulate numbers (pseudo-) randomly from the interval (0,1).  As we will see in Section 5.7 of the [STAT1004 notes](https://moodle.ucl.ac.uk/pluginfile.php/3054127/mod_resource/content/22/STAT1004notes-2x1.pdf) this is equivalent to generating numbers that behave approximately like a random sample from a standard uniform U(0,1) distribution.
...
In the [Challenger Space Shuttle disaster](stat1004-ch1a-shuttle-vignette.html) vignette we needed to simulate numbers from a binomial distribution with parameters $6$ and $p$, for a given value of $p$.  As we will see in Section 5.3 of the [STAT1004 notes](https://moodle.ucl.ac.uk/pluginfile.php/3054127/mod_resource/content/22/STAT1004notes-2x1.pdf) this distribution arises as the number of 'successes' (perversely, in the space shuttle example thermal distress of an O-ring is a 'success') in a fixed number (here, this is 6) of independent trials, where each trial results either in a 'success' or a 'failure'.
...
A related point is that is we turn the logical vector `distress` (which contains `TRUE`s and `FALSE`s) into a numeric vector, then we get ...
...
The exponential distribution (see Section 5.6 of the [STAT1004 notes](https://moodle.ucl.ac.uk/pluginfile.php/3054127/mod_resource/content/22/STAT1004notes-2x1.pdf)) is an example of a probability distribution of a continuous random variable (see Section 4.2 of the [STAT1004 notes](https://moodle.ucl.ac.uk/pluginfile.php/3054127/mod_resource/content/22/STAT1004notes-2x1.pdf)).
...
(see Section 2.5 of the [STAT1004 notes](https://moodle.ucl.ac.uk/pluginfile.php/3054127/mod_resource/content/22/STAT1004notes-2x1.pdf)) and superimpose the probability density function (p.d.f.) of the exponential distribution from which these values have been simulated.
...
After you have answered Question 5 of the [Quiz for Exercises 6](https://moodle.ucl.ac.uk/mod/quiz/view.php?id=2671329) you will see why the following R code could be used to simulate from an exponential distribution.
...
See the 
[Stochastic simulation video](https://moodle.ucl.ac.uk/mod/url/view.php?id=2180273) for some detail.  The following code implements this for the normal distribution
(see Section 5.7 of the [STAT1004 notes](https://moodle.ucl.ac.uk/pluginfile.php/3054127/mod_resource/content/22/STAT1004notes-2x1.pdf)).
...
We the use of `ru` to simulate from a multivariate distribution using the bivariate normal distribution. This distribution will be studied in the second-year statistics module [STAT2001](https://moodle.ucl.ac.uk/course/view.php?id=2907).  See the Wikipedia page for the 
...
The generalized ratio-of-uniforms method works well enough for this example but it is not the most efficient way to simulate from a multivariate normal distribution.  A better function is the `mvrnorm` in the MASS package [@MASS].
